...
function writeMessage() {
   ...
   var message = $(".theTextInput").val();
     var name = "INSTRUCTOR : "+teacherName;
    

  firebase.firestore().collection('liveChat').doc(univCourseID).collection("messages").add({
    message: message,
    userID: userID,
    name:name,
...
docRef.get().then(function(doc) {
    if (doc.exists) {
        ...
        teacherName = teacherData.Details[1];
...
docRef.get().then(function(doc) {
    if (doc.exists) {
       var userData = doc.data();
        var courseID = userData.courseID;
         teacherName = userData.Details[1];
